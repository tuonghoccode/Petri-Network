...
function question_1() {
  ...
  document.querySelector(
    ".current_1"
  ).innerHTML = `Current Marking: [${wait_form.value}.wait,${inside_form.value}.inside,${done_form.value}.done]`;
  ...
  document.querySelector(".start-1").addEventListener("click", function () {
    if (start_trans.prev.data >= 1 && start_trans.data == 0) {
      start_trans.prev.data--;
      start_trans.data = 1;
      document.querySelector(
        ".current_1"
      ).innerText = `Current Marking: [${wait_token.data}.wait,${inside_token.data}.inside,${done_token.data}.done]`;
      document.querySelector(".start-1").classList.add("fired");
      inside_token.data++;
      document.querySelector(".wait").innerHTML = wait_token.data;
    }
    if (start_trans.data == 1) {
      setTimeout(function () {
        document.querySelector(".start-1").classList.remove("fired");
        document.querySelector(".inside").innerHTML = inside_token.data;
        document.querySelector(
          ".current_1"
        ).innerHTML = `Current Marking: [${wait_token.data}.wait,${inside_token.data}.inside,${done_token.data}.done]`;
        start_trans.data = 0;
      }, time);
    }
  });
  document.querySelector(".change-1").addEventListener("click", function () {
    if (change_trans.prev.data >= 1 && change_trans.data == 0) {
...
function question_2() {
  ...
  document.querySelector(
    ".current_2"
  ).innerHTML = `Current Marking: [${free_token}.free,${busy_token}.busy,${docu_token}.docu]`;
  document.querySelector(".end").addEventListener("click", function () {
    if (docu_token >= 1 && end == 0) {
      docu_token--;
      end = 1;
      document.querySelector(
        ".current_2"
...
function question_3() {
  ...
  document.querySelector(
    ".current_3"
  ).innerHTML = `Current Marking: [${wait_token}.wait,${free_token}.free,${inside_token}.inside,${busy_token}.busy, ${done_token}.done,${docu_token}.docu]`;
  ...
  document.querySelector(".start-3").addEventListener("click", function () {
    if (free_token >= 1 && wait_token >= 1 && start == 0 ) {
      free_token--;
      wait_token--;
      start = 1;
      document.querySelector(
  ...
  document.querySelector(".change-3").addEventListener("click", function () {
    if (busy_token >= 1 && inside_token >= 1 && change == 0) {
      busy_token--;
      inside_token--;
      change = 1;
      document.querySelector(
